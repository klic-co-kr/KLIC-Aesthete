...
import { regionsToNodes } from '../hooks/region-provider.mjs';
...
export function importHtml(htmlText, opts = {}) {
  // Render-hook path (Phase 3): if a browser hook already rendered the page and read every
  // element's bbox (incl. flex/grid — which the pure-JS parse below can't see), consume those
  // regions directly. The browser is the source of truth when supplied.
  if (Array.isArray(opts.regions) && opts.regions.length) {
    const canvas = opts.canvas || { w: opts.w || 1200, h: opts.h || 800 };
    return {
      schema_version: 1,
      diagram_type: 'layout',
      meta: { title: 'html import (rendered regions)', canvas, source: 'html' },
      nodes: regionsToNodes(opts.regions),
    };
  }
