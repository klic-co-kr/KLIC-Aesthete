...
//   - the ELEMENTS (regions of interest) are DECLARED as a `regions` list (annotated bounding
//     boxes) — produced out-of-band by a render hook (lib/hooks/sam-hook.mjs, vision model) and
//     passed in via opts. Without regions you get the canvas only.
...
export function importImage({ imageBuffer, regions, canvas } = {}) {
  ...
  const nodes = Array.isArray(regions) ? regionsToNodes(regions) : [];
  const note = !regions
