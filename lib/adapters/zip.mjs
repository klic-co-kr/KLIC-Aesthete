...
export function readZip(buf) {
  ...
  for (let e = 0; e < cdCount; e++) {
    ...
    const compMethod = view.getUint16(cdOffset + 10, true); // ZIP CD: +10 is compression method (+8 is the flags)
...
export function writeZip(entries) {
  ...
  for (const { name, data } of entries) {
    ...
    cd.writeUInt16LE(method, 10);      // compression method (ZIP CD: +10, not +8)
