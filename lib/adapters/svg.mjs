...
export function importSvg(svgText) {
  ...
  // Stash the import-time bbox per node so overlay export can derive the delta (fixed bbox vs
  // original) WITHOUT breaking the closed loop: fix still mutates `bbox` (measure reads bbox),
  // overlay reads `_originalBbox` to compute the transform wrap.
  const nodes = leaves.map(({ node }) => ({ ...node, _originalBbox: { ...node.bbox } }));
  return {
    schema_version: 1,
    diagram_type: 'layout',
...
// SHARED by importSvg and lib/overlay/svg.mjs so overlay can map fixed nodes back to their source
// elements by matching the import-time bbox. Deterministic: same text → same leaf order/bboxes.
export function parseSvgLeaves(svgText) {
  ...
  return { svg, w, h, leaves };
}

function walk(el, off, pairs, nextSeq) {
