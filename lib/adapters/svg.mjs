...
// `tainted` flags a non-translate parent transform (scale/rotate/matrix/skew): the translate-only
// delta accumulation makes the bbox wrong, so overlay must bail rather than emit a silent mis-wrap.
export function parseSvgLeaves(svgText) {
  ...
  let seq = 0;
  walk(svg, { tx: 0, ty: 0 }, pairs, () => seq++, ctx);
...
function walk(el, off, pairs, nextSeq, ctx) {
  for (const c of el.children) {
    ...
    if (tr && /scale|rotate|matrix|skew/i.test(tr)) ctx.tainted = true; // non-translate → bbox unreliable
    ...
    if (tag === 'g' || c.children.some((gc) => localName(gc.tag) !== undefined)) {
      walk(c, delta, pairs, nextSeq, ctx);
    }
  }
}
