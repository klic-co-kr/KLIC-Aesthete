...
import { containsBox, backdropIndexFor } from '../geometry.mjs';
...
// Transform a single point by the composed matrix (transformBox's math, one corner). Recorded
// endpoints must land in the same world space as bboxes so anchor depth is measured in ONE
// coordinate system — a connector under a translate/scale parent compares against its host's
// world bbox (rounded to 4 decimals like every other world coordinate here). Returns an [x, y]
// tuple, the schema shape for endpoints entries.
function transformPoint(x, y, matrix) {
  return [
    Number((matrix[0] * x + matrix[2] * y + matrix[4]).toFixed(4)),
    Number((matrix[1] * x + matrix[3] * y + matrix[5]).toFixed(4)),
  ];
}
...
// First/last ON-PATH points + subpath count of a <path d="...">, mirroring pathBbox's token
// walk (M/L/H/V/C/S/Q/T/A/Z, relative + absolute; arcs use their endpoint; control points are
// not on-path). These two points are the connector's anchor candidates for vuln anchor-buried —
// a buried arrowhead is judged where the stroke actually starts/ends, not at the bbox edge.
export function pathEndpoints(d) {
  if (!d) return null;
  const tokens = String(d).match(/[mlhvcsqtazMLHVCSQTAZ]|-?[0-9.]+(?:e[-+]?[0-9]+)?/gi);
  if (!tokens || !tokens.length) return null;
  let x = 0; let y = 0; let sx = 0; let sy = 0;
  let first = null; let last = null; let subpaths = 0;
  let i = 0;
  const take = (n) => { const out = []; while (n-- > 0 && i < tokens.length) out.push(Number(tokens[i++])); return out; };
  while (i < tokens.length) {
    const t = tokens[i];
    ...
    if (C === 'M' || C === 'L') {
      ...
      while (p.length === 2) {
        const [a, b] = p; x = rel ? x + a : a; y = rel ? y + b : b;
        if (C === 'M' && isSubpathStart) { subpaths++; sx = x; sy = y; isSubpathStart = false; }
        if (!first) first = [x, y];
        last = [x, y];
        if (C === 'L' || i >= tokens.length || /[a-zA-Z]/.test(tokens[i])) break;
        p = take(2); // implicit L after M
      }
    } else if (C === 'H') { let p = take(1); while (p.length === 1) { x = rel ? x + p[0] : p[0]; last = [x, y]; if (i >= tokens.length || /[a-zA-Z]/.test(tokens[i])) break; p = take(1); } }
    else if (C === 'V') { let p = take(1); while (p.length === 1) { y = rel ? y + p[0] : p[0]; last = [x, y]; if (i >= tokens.length || /[a-zA-Z]/.test(tokens[i])) break; p = take(1); } }
    else if (C === 'C') { const [, , , , X, Y] = take(6); x = rel ? x + X : X; y = rel ? y + Y : Y; last = [x, y]; }
    else if (C === 'S') { const [, , X, Y] = take(4); x = rel ? x + X : X; y = rel ? y + Y : Y; last = [x, y]; }
    else if (C === 'Q') { const [, , X, Y] = take(4); x = rel ? x + X : X; y = rel ? y + Y : Y; last = [x, y]; }
    else if (C === 'T') { const [X, Y] = take(2); x = rel ? x + X : X; y = rel ? y + Y : Y; last = [x, y]; }
    else if (C === 'A') { take(7); /* rx,ry,xrot,laf,sf,X,Y — endpoint only */ const end = tokens.slice(i - 2, i).map(Number); if (end.length === 2) { x = rel ? x + end[0] : end[0]; y = rel ? y + end[1] : end[1]; last = [x, y]; } }
    else if (C === 'Z') { x = sx; y = sy; last = [x, y]; }
  }
  if (!first) return null;
  return { first, last, subpaths };
}
...
export function parseSvgLeaves(svgText) {
  ...
  // BEFORE dropping full-canvas backdrops: they are exactly what most text sits on, so the
  // paint-stack lookup has to still see them.
  resolveTextBackdrops(pairs.map(({ node }) => node));
...
// SVG gives no element its backdrop color — it falls out of the paint stack. Resolve each text
// node's style.bg to the nearest enclosing filled shape beneath it (page rect, card, panel), so
// WCAG contrast is computed against the surface the glyphs actually sit on. Text with nothing
// beneath it keeps the '#ffffff' default set in geomNode (no surface to read).
// Mutates in place — nodes are freshly built by this walk and not yet observable.
function resolveTextBackdrops(nodes) {
  for (let i = 0; i < nodes.length; i++) {
    if (nodes[i].kind !== 'text') continue;
    const j = backdropIndexFor(nodes, i);
    if (j >= 0) nodes[i].style.bg = nodes[j].style.bg;
  }
}
...
function geomNode(c, tag, matrix, nextSeq, viewport) {
  ...
  let localBox = null;
  let localEnds = null;   // candidate stroke endpoints in LOCAL coords (world-transformed below)
  let subpaths = 0;       // <path> only: M-command count (multi-subpath → no anchor capture)
  ...
  if (tag === 'rect' || tag === 'image' || tag === 'use') {
    ...
    localEnds = [[x1, y1], [x2, y2]];
  } else if (tag === 'path') {
    const pb = pathBbox(a.d);
    if (!pb) return null;
    localBox = pb;
    const pe = pathEndpoints(a.d);
    if (pe) { localEnds = [pe.first, pe.last]; subpaths = pe.subpaths; }
  } else if (tag === 'text') {
  ...
  // Stroke-endpoint capture (raw input for vuln anchor-buried). A <line> always records its two
  // ends; a <path> only when it is stroke-style line art (fill absent/"none") with a SINGLE
  // subpath whose ends do not coincide — filled shapes, closed loops and multi-subpath compound
  // paths carry no connector anchor semantics, so they get no key at all (graceful absence,
  // same pattern as strokeWidth on strokeless nodes). Endpoints are world coords, rounded to
  // 4 decimals like the bbox.
  let endpoints = null;
  if (localEnds && (tag === 'line' || (fill === null && subpaths === 1))) {
    const pts = localEnds.map(([ex, ey]) => transformPoint(ex, ey, matrix));
    if (tag === 'line' || Math.hypot(pts[0][0] - pts[1][0], pts[0][1] - pts[1][1]) > 0.5) {
      endpoints = pts;
    }
  }
  ...
  const rawStrokeWidth = a['stroke-width'] != null ? num(a['stroke-width']) : 1;
  // Gap B: stroke thickness in canvas units (same rule as fontSize: raw × matrixScale,
  // default 1) so collision can inflate a thin connector's line bbox into its real ink band.
  const strokeWidth = stroke ? Number((rawStrokeWidth * matrixScale).toFixed(3)) : undefined;
  ...
  const role = tag === 'text' ? (fs >= 22 ? 'heading' : 'body') : 'decor';
  ...
  return {
    ...
    bbox,
    ...(endpoints ? { endpoints } : {}),
    // ALT color contract (same as lib/adapters/html.mjs): style.color = FOREGROUND,
    // style.bg = BACKDROP. For a shape, its own fill IS its background. For <text>, the fill is
    // the GLYPH color (foreground) — putting it in bg made hierarchy compare a constant against
    // the glyph and fail every legible SVG on WCAG contrast. The real backdrop is not knowable
    // per-element here; resolveTextBackdrops() fills it in from the paint stack after the walk.
    style: tag === 'text'
      ? { fontSize: fs, luminance: Number(luminance.toFixed(3)), opacity, color: fill || stroke || '#111827', bg: '#ffffff', filled: fill !== null, role, ...(strokeWidth !== undefined ? { strokeWidth } : {}) }
      : { fontSize: fs, luminance: Number(luminance.toFixed(3)), opacity, color: stroke || '#111827', bg: fill || '#ffffff', filled: fill !== null, role, ...(strokeWidth !== undefined ? { strokeWidth } : {}) },
  };
}

function sameSizeShadow(a, b) {
  const minDim = Math.min(a.w, a.h, b.w, b.h);
  const sizeTolerance = Math.max(1, minDim * 0.02);
  // Shadows/tints are offset by a few pixels of blur direction. The former 12px / 12%-of-min-dim
  // tolerance also swallowed genuinely overlapping same-size cards and demoted one to decor,
  // hiding a real P0 collision. Restrict to a shadow-scale offset so only true backdrop/shadow
  // stacks match; larger offsets between equal rects stay independent collision targets.
  const offsetTolerance = Math.max(4, minDim * 0.03);
