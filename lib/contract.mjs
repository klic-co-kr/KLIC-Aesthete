...
//
// Slop note (spec §6 M2): slop is NOT in the default contract. v1 slop gating is via
// `aesthete-post --slop-gate`, which drives the slop fold inside foldDecision() at priority 60
// (lib/skill-decision.mjs) — mirroring vuln, slop flows through a separate `slopReport`, NOT
// through report.skills.slop. skill-post never populates report.skills.slop, so a slop contract
// criterion here would silently no-op (getMetric → NaN → forever 'unmeasured'). Use the slop
// fold, not a contract criterion.
