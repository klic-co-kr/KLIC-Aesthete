...
const MANIFEST_CHANGE_CODES = new Set(STALE_CODE_ORDER.slice(5));
...
function validateBindingStructure(decision, version) {
  ...
  if (
    binding.schema !== `aesthete.binding/${version}`
    ...
    || (isV2 && !validateIntent(binding.intent))
  ) {
    result.receiptInvalid = true;
  }
...
function validateReceiptVersionShape(decision, version) {
  ...
  const structural = validateBindingStructure(decision, version);
...
export function validateReceiptV1Shape(decision) {
  return validateReceiptVersionShape(decision, 'v1');
}

export function validateReceiptV2Shape(decision) {
  return validateReceiptVersionShape(decision, 'v2');
}
...
function validateCurrent(current, decision, version) {
  const isV2 = version === 'v2';
  ...
  if (
    !LOWER_HEX_256.test(current.artifact_sha256)
    || !validateContract(current.contract)
    || (isV2 && !validateIntent(current.intent))
  ) {
    return false;
  }
...
export function verifyDecisionBinding(decision, current) {
  ...
  const version = decision.binding.schema === 'aesthete.binding/v2' ? 'v2' : 'v1';
  if (!validateCurrent(current, decision, version)) {
  ...
  return {
    ...
    issues: ordered,
    checked: [...(version === 'v2' ? ALL_CHECKED_V2 : ALL_CHECKED)],
  };
}
