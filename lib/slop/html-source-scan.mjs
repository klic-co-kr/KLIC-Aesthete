...
// --- declaration-literal fields (2026-08 absorption of @gessobuild/anti-slop v0.4.2, MIT) ---
// Still literal-presence (spec §5 M1): raw declaration VALUES only, no cascade resolution,
// no selector→element application. Signatures own every interpretation decision.
const REPEATING_GRAD_RE = /repeating-(?:linear|radial|conic)-gradient\([^;}]*/gi;
const FONT_FAMILY_RE = /font-family\s*:\s*([^;}]+)/gi;
const GOOGLE_FONT_FAMILY_RE = /family=([^&"'>]+)/gi; // applied over fonts.googleapis.com URLs
const RADIUS_RE = /border-radius\s*:\s*([^;}]+)/gi;
const SHADOW_RE = /(?:-webkit-)?(?:box|text)-shadow\s*:\s*([^;}]+)/gi;
const DROP_SHADOW_RE = /drop-shadow\(\s*((?:[^()]|\([^()]*\))*)\)/gi; // paren-aware: a nested rgba()/hsl() color is one level deep
// HTML comments are not rendered — text/heading/img extraction runs on a comment-stripped
// source so commented-out markup never fires a visible-copy or imagery signature. Decl and
// carrier extraction stays on the RAW source: declarations are authorship evidence either
// way, and carrier counts MUST match what stripCarriers removes (same counter contract).
const COMMENT_RE = /<!--[\s\S]*?-->/g;

// Dash entity forms (named + numeric, semicolon optional) → raw codepoints, so dash RUNS
// written as entities land in decorative-divider and singles in copy.em-dash — the same
// boundary that raw characters already follow. Digit guards keep `&#82125;` from matching
// `&#8212` + stray `5`.
export function normalizeDashEntities(s) {
  return String(s)
    .replace(/&(?:mdash(?![a-z0-9]);?|#0*8212(?![0-9]);?|#x0*2014(?![0-9a-f]);?)/gi, '—')
    .replace(/&(?:ndash(?![a-z0-9]);?|#0*8211(?![0-9]);?|#x0*2013(?![0-9a-f]);?)/gi, '–');
}
const TRANSITION_RE = /(?:-(?:webkit|moz|o|ms)-)?transition(?:-property)?\s*:\s*([^;}]+)/gi;
...
// Generalized sibling of borderContextsOf: EVERY declaration context (rule body from <style>
// blocks — with the selector it belongs to — plus each inline style attribute), for signatures
// that need same-block co-occurrence (hollow-text) or selector-shape checks (body-display-
// contents). Literal-presence only: no cascade, no element resolution.
function declContextsOf(src) {
  const contexts = [];
  for (const sm of src.matchAll(/<style\b[^>]*>([\s\S]*?)<\/style\s*>/gi)) {
    for (const rm of (sm[1] || '').matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
      if (rm[2]) contexts.push({ selector: (rm[1] || '').trim(), body: rm[2], inline: false });
    }
  }
  for (const im of src.matchAll(/style\s*=\s*["']([^"']*)["']/gi)) {
    if (im[1]) contexts.push({ selector: null, body: im[1], inline: true });
  }
  return contexts;
}

// <body> display:contents — the one display bug worth a v1 literal check. Selector-LIST aware:
// `body {}` and `html, body {}` count (body is a standalone selector member); `body .child {}`
// does not (descendant), and `<body style="display:contents">` is the inline form.
function bodyDisplayContentsOf(src, contexts) {
  if (/<body\b[^>]*style\s*=\s*["'][^"']*display\s*:\s*contents/i.test(src)) return true;
  return contexts.some((c) => !c.inline
    && /display\s*:\s*contents/i.test(c.body)
    && c.selector.split(',').some((part) => part.trim().toLowerCase() === 'body'));
}
...
export function scanHtmlSource(html) {
  ...
  const measuredNotes = [];
  // Comment-stripped twin of src for every VISIBLE-content extraction (headings, text
  // samples, img tags). See COMMENT_RE for why declarations/carriers stay on raw src.
  const markup = src.replace(COMMENT_RE, ' ');
  ...
  const headings = [...markup.matchAll(HEADING_RE)].map((m) => {
    const raw = m[2] || '';
  ...
  // --- Gesso-ported declaration-literal fields (values raw; signatures interpret) ---
  // Extracted per declaration CONTEXT (style rule body or one inline style attribute) — never
  // from raw src — so an inline value can never run past its closing quote.
  const declContexts = declContextsOf(src);
  const styleBlocks = declContexts.map((c) => c.body);
  const declsOf = (re) => {
    const out = [];
    for (const c of declContexts) {
      for (const m of (c.body || '').matchAll(re)) out.push((m[1] || '').trim());
    }
    return out;
  };
  const repeatingGradients = (src.match(REPEATING_GRAD_RE) || []).map((s) => s.trim());
  // Font families: font-family declaration values (comma-split, quote-stripped, lowercase) +
  // Google Fonts URL family= params ('+'-decoded, ':'-modifiers dropped).
  const fontFamilies = [];
  for (const v of declsOf(FONT_FAMILY_RE)) {
    for (const fam of v.split(',')) {
      const name = fam.trim().replace(/^["']|["']+$/g, '').toLowerCase();
      if (name) fontFamilies.push(name);
    }
  }
  for (const lm of src.matchAll(/fonts\.googleapis\.com[^"'>]*/gi)) {
    for (const fm of (lm[0] || '').matchAll(GOOGLE_FONT_FAMILY_RE)) {
      // Malformed percent-encoding in a URL must degrade to the raw param, never crash the
      // scan (this runs in the scanner, outside every signature's try/catch).
      let param = fm[1] || '';
      try { param = decodeURIComponent(param); } catch { /* raw param is the safe fallback */ }
      const name = param.split(':')[0].replace(/\+/g, ' ').trim().toLowerCase();
      if (name) fontFamilies.push(name);
    }
  }
  const radiusDecls = declsOf(RADIUS_RE);
  const shadowDecls = declsOf(SHADOW_RE);
  const dropShadows = declsOf(DROP_SHADOW_RE);
  const transitionDecls = declsOf(TRANSITION_RE);
  const willChangeDecls = declsOf(WILL_CHANGE_RE);
  const imgTags = (markup.match(IMG_TAG_RE) || []).slice();
  const bodyDisplayContents = bodyDisplayContentsOf(src, declContexts);
