...
// 2026-08 decoration/motion/layout expansion — declaration-literal signatures
// (hollow-text, dark-glow, over-rounded, repeating-stripe, decorative-divider,
// transition-all, will-change-misuse, layout-prop-anim, body-display-contents).
// All read raw declaration VALUES from the scanner; interpretation lives here.
// A per-element `data-slop-allow`-style opt-out is deliberately absent — our findings are
...
// Count neon-glow layers (dark-glow): non-inset shadow layer whose color is
...
export const SIGNATURES = [
  ...
  {
    // hollow-text: can fail WCAG outright (wherever the stroke is not painted, the text is
    // invisible). Stroke declaration + a transparent COLOR/FILL in the SAME declaration block
    // (rule body or one inline style) — co-occurrence is per-block, so a transparent overlay
    // elsewhere on the page never armors an unrelated stroke. `background: transparent`
    // beside a stroke does NOT fire (the glyph still has its fill).
  ...
  {
    // dark-glow: the neon halo behind buttons/bento tiles — the signature move of generated
    // "premium dark SaaS". Neutral elevation shadows carry no chroma and never match;
    // drop-shadow() filters count too.
  ...
  {
    // over-rounded: SINGLE-value px radius in the 40–120 band; pills (9999px) and circles
    // (50%) fall outside by convention, and per-corner multi-value radii are excluded (a
    // deliberate asymmetric corner is a different decision). A filled-surface requirement
    // is unresolvable in a literal scan (no cascade), so an unfilled wrapper with a blob
    // radius is a documented residual.
  ...
  {
    // repeating-stripe: texture without intent — the CSS equivalent of construction tape.
    // Deliberately separate from palette.gradient: the GRAD_RE superset the cliché-hue rule
    // reads still contains these (compat), and this rule fires on the repeating FORM
    // regardless of hue.
  ...
  {
    // decorative-divider: runs of 2+ box-drawing characters (U+2500–U+2570, U+2574–U+257F —
    // diagonals excluded) or 2+ em/en dashes in VISIBLE text: how a language model draws a
    // line when it cannot draw a line. A single dash is copy.em-dash's finding, never this
    // one's. ASCII '---' runs are NOT included — in HTML source they live overwhelmingly
    // inside comments, which the visible-text haystack already excludes, and rendered
    // markdown hr's arrive as <hr>.
  ...
  {
    // transition-all: `transition: all` animates every property that happens to change and
    // forces the browser to watch everything (narrowing the list is a design decision this
    // rule refuses to make). P2 (not P1) deliberately: this one is common in hand-written
    // code too — it is a smell, not a generated-only tell.
  ...
  {
    // will-change-misuse: will-change pointed at layout/paint properties allocates a GPU
    // layer that cannot accelerate anything — performance cargo cult residue. Compositable
    // set + spec values + CSS-wide keywords stay silent.
  ...
  {
    // layout-prop-anim: transitioning layout properties reflows the page every frame — the
    // motion stutters exactly where it tries to impress (the right rewrite depends on intent,
    // so this detects only). `transition: all` is transition-all's finding and is excluded
    // here.
  ...
  {
    // body-display-contents: display:contents on <body> discards its padding/width/flex-gap —
    // no side padding, content under any fixed bar, zero section rhythm. Legitimate on nested
    // wrappers, never on body; the scanner's selector-list check keeps `body .child {}` out.
