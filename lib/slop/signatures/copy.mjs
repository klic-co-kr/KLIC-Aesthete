...
// Visible-copy haystack shared by the Gesso-ported copy signatures: body samples (TEXT_TAGS)
...
// 2026-08 absorption of @gessobuild/anti-slop v0.4.2 (MIT), copy axis — literal-regex ports.
...
export const SIGNATURES = [
  ...
  {
    // Ported from anti-slop "lorem-ipsum" (their GATE, sev 2 — "shipping it is always wrong").
    // Both phrases, case-insensitive; either alone fires. P1 here: an unambiguous
    // generated-and-never-reviewed artifact, not a judgment call.
    id: 'slop.copy.lorem',
  ...
  {
    // Ported from anti-slop "em-dash-copy". Single em dash only (raw U+2014, &mdash;, &#8212;,
    // &#x2014;) or a SPACED en dash used as an em-dash substitute; unspaced ranges (Mon-Fri)
    // are legitimate. Runs of 2+ dashes are decoration.decorative-divider's finding. P2
    // advisory (their FIX): editorial English earns one occasionally — the TELL is density,
    // and minHits is the knob for that judgment.
  ...
  {
    // Ported from anti-slop "oversized-number": ≥10,000 rendered raw (comma-grouped or 5+
    // digits), currency symbol optional. %/K/M/B-suffixed numbers are spared (abbreviated, or
    // fake-precision's neighborhood). FP stance: raw comma-grouped amounts are standard
    // typography in some locales (e.g. Korean 원 amounts) — this stays P2 advisory for that
    // reason; dashboards that should abbreviate are the target, not financial documents.
  ...
  {
    // Ported from anti-slop "not-x-but-y-cadence" (their FLAG → P2 here). The manufactured
    // rebuttal rhythm: "it's not (just) X, it's Y" — the single most recognized generated-copy
    // cadence; contrast as a tic. Occasionally a writer earns it, so it reports for judgment.
  ...
  {
    // Ported from anti-slop "apologetic-error-copy" (their GATE). "Oops"/"Whoops"/"Uh oh"/
    // "something went wrong" — the template default error that names no failure and offers no
    // next step. Errors are guidance surfaces, not mood management.
  ...
  {
    // Ported from anti-slop "live-clock-eyebrow". WHOLE-TEXT equality on a sampled text
    // element: optional leading status dot, LIVE/NOW token, optional separator + clock time.
    // The equality is the safety — "Departs 09:41", "LIVE STREAM setup", and any time not
    // fronted by LIVE/NOW never match. Sampled TEXT_TAGS (p/li/span/button/a/td) are a subset
    // of their eligible small-element set; heading-tag eyebrows are out (headings are excluded
    // upstream by whole-text shape anyway).
