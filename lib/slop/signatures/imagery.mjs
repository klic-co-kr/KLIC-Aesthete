// slop imagery signatures — 5th sub-axis, opened 2026-08 by the @gessobuild/anti-slop v0.4.2
// (MIT) absorption. P1 = placeholder-service <img> src (lorem ipsum for pictures) and
// broken/template src (missing, empty, mustache slot, path/to/). Reads the scanner's raw
// <img> tags; every src interpretation lives here, not in the scanner.
...
// Placeholder-service hosts (anti-slop "placeholder-image" list, v0.4.2). Real photo CDNs
// (images.unsplash.com, images.pexels.com) are deliberately absent — a real photo is a real
// photo regardless of CDN. The leading \b keeps the match from starting mid-DNS-label
// ('notpicsum.photos' is one label and must not fire). Documented residual: a hyphenated
// look-alike label ('evil-picsum.photos') still matches — a real domain squat, not HTML copy.
...
// template-slot and stand-in patterns (anti-slop "broken-image").
...
// Resolver slots a host pipeline sanctions and fills after generation (anti-slop list):
...
export const SIGNATURES = [
  {
    // Ported from anti-slop "placeholder-image" (their GATE). P1: placeholder imagery is the
    // single strongest "this screen was never finished with real assets" tell, and several of
    // these services rotate or die — the design literally changes under you.
  ...
  {
    // Ported from anti-slop "broken-image" (their FIX). A missing image beats a broken one:
    // missing/empty src, template mustache slots, path/to/ stand-ins, example.com placeholder
    // hosts. Resolver-slot imgs (data-photo-query etc.) are sanctioned — a pipeline fills them.
