 ...
 * @param {object|null} [input.slopReport] - scanSlop() result (spec §6)
 * @param {boolean} [input.slopGate] - when true, P1 slop findings escalate to regenerate
 * @param {boolean} [input.slopAutofix] - reserved (future autofix hook)
...
export function foldDecision(input = {}) {
  ...
  if (input.slopReport) {
    ...
    if (measured && p0.length) {
      ...
      for (const f of p0) {
        reasons.push({
          code: `SLOP_P0_${String(f.id.split('.').pop()).toUpperCase().replace(/[^A-Z0-9]+/g, '_')}`,
          tier: 'P0',
          detail: f.title || f.id,
          fixable: false,
        });
      ...
      for (const f of p1) {
        reasons.push({
          code: `SLOP_P1_${String(f.id.split('.').pop()).toUpperCase().replace(/[^A-Z0-9]+/g, '_')}`,
          tier: 'P1',
          detail: f.title || f.id,
          fixable: false,
        });
      ...
      for (const u of p0Unmeasured) {
        reasons.push({
          code: `SLOP_P0_UNMEASURED_${String(u.id.split('.').pop()).toUpperCase().replace(/[^A-Z0-9]+/g, '_')}`,
          tier: 'P0',
      ...
      for (const f of findings.filter((x) => x.tier === 'P2').slice(0, 5)) {
        reasons.push({
          code: `SLOP_ADVISORY_${String(f.id.split('.').pop()).toUpperCase().replace(/[^A-Z0-9]+/g, '_')}`,
          tier: 'advisory',
          detail: f.title || f.id,
          fixable: false,
        });
  ...
  if (input.contractRequested && input.contractEval && !input.contractEval.allPass) {
    ...
    if (fails.length) {
      // A failing criterion is a REAL, regeneration-worthy violation only if its skill produced a
      // finite measured value — i.e. coverage was measurable enough to actually compare. A criterion
      // whose status is 'unmeasured' (skill coverage === 'unmeasurable') or whose measured is null
      // (partial coverage that yielded no metric) is a MEASUREMENT GAP, not a defect the generator
      // can fix: the engine is deterministic, so re-generation re-measures to the identical gap and
      // the loop never converges. Such criteria must NOT drive regenerate. They also must NOT pass
      // vacuously (§12 anti-false-pass). Resolution: attach the reason, but escalate to human so a
      // real person decides — unless at least one real metric fail coexists, in which case the
      // regeneration-worthy violation wins as before.
      const isInsufficient = (c) =>
        c.status === 'unmeasured' ||
        c.measured === null || c.measured === undefined || !Number.isFinite(c.measured);
      const realFails = fails.filter((c) => !isInsufficient(c));
      const insufficientFails = fails.filter(isInsufficient);
      for (const c of realFails.slice(0, 12)) {
        reasons.push({
          code: 'CONTRACT_FAIL',
          tier: 'P2',
      ...
      for (const c of insufficientFails.slice(0, 12)) {
        reasons.push({
          code: 'CONTRACT_UNMEASURABLE',
          tier: 'P1',
          detail: `${c.criterion || `${c.skill}.${c.metric}`} — axis unmeasurable/partial, not verifiable from geometry`,
          fixable: false,
        });
      }
      if (realFails.length) {
        ...
        // every failing criterion is measurement-insufficient → cannot certify pass (§12) yet
        // regeneration cannot help. Escalate to human even when coverageScore > 0.
        candidates.push({ priority: PRI.human_coverage, decision: 'human' });
      }
    }
  }
