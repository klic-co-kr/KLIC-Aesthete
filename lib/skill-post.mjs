...
import { importBuffer } from './adapters/index.mjs';
...
function normalizePostFlags(flags = {}) {
  return {
    domain: flags.domain === undefined || flags.domain === null || flags.domain === ''
      ? undefined
      : flags.domain,
...
function normalizeRunContext(deps = {}) {
  ...
  try {
    root = path.resolve(deps.root || skillRoot());
  } catch (error) {
    throw new ReceiptInputError(
  ...
  return {
    ...
    runtime,
    executable,
    io: createOperationIo(deps.io || DEFAULT_IO),
...
export function loadArtifact(inputPath, flags = {}, deps = {}) {
  ...
  const io = createOperationIo(deps.io || DEFAULT_IO);
  const snapshot = snapshotArtifact(inputPath, {
...
export async function runPost(inputPath, opts = {}) {
  const flags = normalizePostFlags(opts.flags || {});
  const context = normalizeRunContext(opts.deps || {});
  ...
  const artifact = snapshotArtifact(inputPath, {
    domain: flags.domain,
    slide: flags.slide,
  }, context.io);
  let alt;
  ...
  if (decision.decision === 'fix_geometry') {
    ...
    const action = buildFixAction({
      ...
      artifactPath: path.resolve(inputPath),
