...
function normalizePostFlags(flags = {}) {
  ...
  if (!hasOnlyKeys(flags, [
    'domain', 'slide', 'contract', 'intent', 'profile', 'structure', 'type',
    'lint', 'vuln', 'vuln-gate', 'slop', 'slop-gate', 'slop-autofix',
    'human-on-unfixable',
  ])) {
    throw new ReceiptInputError(
      'POLICY_INPUT_INVALID',
  ...
  return {
    ...
    intentPath: normalizeIntentPath(flags.intent),
...
export async function runPost(inputPath, opts = {}) {
  ...
  return {
    ...
    intentSnapshot: requestedIntent,
  };
}
