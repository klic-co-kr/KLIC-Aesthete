...
// A stroke-only shape paints no field, so its visible color is the STROKE, which adapters put in
// style.color (svg.mjs sets color: stroke for unfilled shapes and leaves bg at the '#ffffff'
// default). Reporting that default made lucide-style outline icons — fill="none" stroke="…" —
// all look identically white to the palette skills.
export function ownColor(node) {
  const style = node?.style;
  const usesForeground = node?.kind === 'text' || style?.filled === false;
  const c = usesForeground ? style?.color : style?.bg;
