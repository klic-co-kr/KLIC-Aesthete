...
// Gap B: measure/fix must agree on what counts as a thin connector and on the band/ink
// geometry their (thin connector × text) judgment uses — these are the collision skill's
// own helpers, so the fixer resolves exactly what the measurement reports.
import { isThinConnector, strokeBand, tangencyShrunk } from './skills/collision.mjs';
...
// Gap B disclosure, written into every *.fix-log.json entry that moved a label out of a
// thin-connector ink band. A label's escape move is a minimal geometric separation; whether
// the label keeps its MEANING (sequence diagrams read y as time — which edge it belongs to)
// is not measurable by the engine and needs human/visual verification.
export const LABEL_MOVE_DISCLOSURE =
  'label escaped a thin-connector ink band with a minimal move; semantic attachment (which edge the label belongs to) cannot be measured by the engine';
...
// Gap B (Phase 1b): the collision skill band-checks (thin stroke-only connector × opaque
// text) pairs on stroke-ink geometry — strokeBand(connector) vs the tangency-shrunk label
// ink — while this fixer used to judge raw bboxes. Two robustness gaps followed: a pair
// whose RAW bboxes clear but whose band still overlaps was never separated (measure/fix
// disagreement), and imported text nodes (filled:false) fell through the stroke–stroke
// skip below, so nothing moved at all. This branch mirrors collision.mjs exactly — same
// helpers, same opacity exemption, band-based push along the min-overlap axis — and the
// LABEL is the preferred mover: a decor connector IS the edge it draws, and moving it cuts
// the diagram's structure (fall back to the connector only when the label is drift-frozen).
function resolveThinBandPair(alt, ctx, a, b) {
  const text = a.kind === 'text' ? a : b;
  const connector = text === a ? b : a;
  // translucency exemption — same threshold as collision.mjs's band branch and its generic guard
  const opText = text.style?.opacity ?? 1;
  const opCon = connector.style?.opacity ?? 1;
  if (opText < 0.5 || opCon < 0.5) return null;
  const band = strokeBand(connector);
  const shrunk = tangencyShrunk(text);
  if (!(shrunk.h > 0) || !rectsOverlap(band, shrunk, 0)) return null;
  const moveText = canMove(ctx, text.id);
  const moveCon = canMove(ctx, connector.id);
  if (!moveText && !moveCon) return null;
  const target = moveText ? text : connector;
  const fixed = target === text ? connector : text;
  const oldB = { ...target.bbox };
  const d = overlapDepth(band, shrunk);
  const minAxis = d.x <= d.y ? 'X' : 'Y';
  const push = Math.min(d.x, d.y) + sepGap(alt.meta.canvas);
  if (minAxis === 'X') {
  ...
  recordMove(ctx, target, oldB);
  if (target === text) {
    ctx.labelMoves.push({ node: text.id, connector: connector.id, disclosure: LABEL_MOVE_DISCLOSURE });
  }
  return target.id;
}
  ...
  for (let k = 0; k < 16; k++) {
    ...
    for (let i = 0; i < alt.nodes.length; i++) {
      for (let j = i + 1; j < alt.nodes.length; j++) {
        ...
        // Gap B 1b: thin connectors are band-checked against opaque text with the SAME
        // geometry the collision skill measures with, and ONLY that check — a raw-bbox
        // judgment here disagrees with the measurement (band ⊃ line bbox, label ink
        // ⊃ shrunk ink), and the stroke–stroke skip below swallows imported text nodes
        // (their adapter style records filled:false), stranding the pair unfixed.
        if ((a.kind === 'text' && isThinConnector(b)) || (b.kind === 'text' && isThinConnector(a))) {
          if (resolveThinBandPair(alt, ctx, a, b)) changed = true;
          continue;
        }
...
export function fixAlt(alt, contract, maxIters = 5, opts = {}) {
  ...
  const ctx = { frozen: new Set(), moveAccum: {}, canvas: alt.meta.canvas, skippedFixes: [], labelMoves: [] };
...
function finalize(outcome, iterations, best, contract, iters, ctx, startCost) {
  ...
  return {
    ...
    // Gap B label-move disclosure — the key is omitted entirely when empty so golden
    // *.fix-log.json snapshots (no thin connectors) stay byte-identical.
    ...(ctx.labelMoves.length ? { labelMoves: ctx.labelMoves } : {}),
...
async function main() {
  ...
  if (flags.slide != null) {
    if (
      typeof flags.slide !== 'string'
      || !/^[1-9][0-9]*$/.test(flags.slide)
    ) {
      throw new Error('--slide must be a positive integer');
    }
    slide = Number(flags.slide);
