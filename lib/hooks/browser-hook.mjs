...
export async function htmlRegions(htmlOrUrl, opts = {}) {
  ...
  try {
    ...
    // Default: LEAF elements only (no element children). A page's DOM tree nests containers around
    // content; extracting every node yields parent/child pairs that overlap BY DESIGN and would
    // register as spurious collisions. Leaves are the atomic arranged units the layout skills should
    // see. Override with opts.selector (e.g. '[data-measure]') to measure a hand-picked set instead.
    const sel = opts.selector || 'body *:not(:has(*))';
