...
// WCAG 2.1 AA minimum text contrast (W3C SC 1.4.3): large text 3:1, everything else 4.5:1.
// "Large" is ≥24px regular OR ≥18px BOLD. ALT carries no fontWeight, so the bold branch is
// UNMEASURABLE here and regular is assumed — an 18px bold heading at 3.5:1 is graded as a
// shortfall it technically passes. Conservative on purpose: never grant an exemption we can't see.
// Unknown font size is treated as small text (the stricter requirement).
const LARGE_TEXT_PX = 24;
const requiredContrast = (fontSize) => (Number.isFinite(fontSize) && fontSize >= LARGE_TEXT_PX ? 3 : 4.5);
...
export default {
  ...
  measure(alt) {
    ...
    for (const n of textNodes) {
      ...
      const bg = n.style?.bg;
      if (fg && bg) {
        // WCAG 2.1 AA (W3C) has TWO text thresholds, not one — grading large display type against
        // 4.5:1 reported legitimate headings as contrast failures.
        contrastSum += Math.min(1, contrastRatio(fg, bg) / requiredContrast(n.style?.fontSize));
        contrastN++;
      }
    }
    ...
    // clarity multiplies TWO dimensions, so coverage is three-way — reporting a half-defaulted
    // clarity as fully `measured` overstates what was seen. PPTX is the live case: pptx.mjs reads
    // no run colors, so contrastAdequacy is a default while stepRegularity is real → partial.
    const haveSteps = sizes.length >= 2;      // stepRegularity has input
    const haveContrast = contrastN > 0;        // contrastAdequacy has input
    const coverage = (!haveSteps && !haveContrast) ? 'unmeasurable'
      : (haveSteps && haveContrast) ? 'measured'
        : 'partial';
    const metrics = {
      clarity,
    ...
    if (clarity < 0.7) {
      ...
      if (contrastAdequacy < 0.7) {
        violations.push({
          ...
          measured: clarity,
          threshold: 0.7,
          message: `hierarchy: contrast inadequate (${contrastAdequacy.toFixed(2)}) — raise fg/bg contrast to WCAG AA (4.5:1 body, 3:1 for text ≥${LARGE_TEXT_PX}px)`,
