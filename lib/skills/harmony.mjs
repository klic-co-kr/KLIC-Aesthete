import { hexToHsl, ownColor } from '../color.mjs';
...
// 명도 창(near-black / near-white 중성 제외). 채도만으로는 부족하다: #111827 같은 본문 글자색은
// s≈0.39로 채도 문턱을 넘지만 지각적으로는 '검정'이며 구성상의 색이 아니다. lib/vuln.mjs의
// ai-cliche-palette가 같은 이유로 lMin/lMax를 쓴다(어두운 네이비 브랜드색은 'AI 그라디언트'가 아니다).
// 근거: 이 창이 없으면 ground-truth 코퍼스 상관이 ρ 0.327 → 0.307로 떨어진다.
const L_MIN = 0.15;
const L_MAX = 0.85;
...
export default {
  ...
  // observe counts what measure() will actually put on the hue wheel — own color, chromatic,
  // inside the lightness window — so the two layers can't disagree about what "colored" means.
  observe(alt) {
    const ns = (alt.nodes || []).filter((n) => {
      if (!n.bbox) return false;
      const c = ownColor(n);
      const hsl = c ? hexToHsl(c) : null;
      return !!hsl && hsl.s >= SAT_THRESHOLD && hsl.l >= L_MIN && hsl.l <= L_MAX;
    });
    return { coloredCount: ns.length };
  },
  measure(alt) {
    const nodes = (alt.nodes || []).filter((n) => n.bbox && (n.style?.opacity ?? 1) >= 0.5);
    const colored = [];
    for (const n of nodes) {
      const own = ownColor(n); // text contributes its GLYPH color, not the surface behind it
      const hsl = own ? hexToHsl(own) : null;
      if (!hsl || hsl.s < SAT_THRESHOLD) continue;            // 무채색/알수없음 제외
      if (hsl.l < L_MIN || hsl.l > L_MAX) continue;           // near-black/near-white 중성 제외
