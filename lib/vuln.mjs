...
import { actualArea, actualShapeComplexity, center, backdropIndexFor, containsBox } from './geometry.mjs';
...
import { ANCHOR_BURIED } from './vuln-sigs/anchor.mjs';
...
export const DEFAULT_THRESHOLDS = {
  ...
  'sibling-misalign': { yOverlap: 0.5, fontClassRatio: 1.1, tolFont: 0.35, minPairs: 2, maxNodes: 80 }, // same-row text siblings whose first baselines disagree — same-font-class pairs only (label/value size mix is a key/value row, not drift)
  'grid-drift': { bandGapRatio: 0.02, leftTolRatio: 0.02, minBands: 3, maxLefts: 2 }, // section left edges drifting across vertical bands; two alternating lefts are a deliberate rhythm
  'label-baseline-off': { minOffsetPx: 2, offsetRatio: 0.08, textShareMin: 0.2, maxNodes: 400 }, // a SOLE label vertically off its box's centre — minOffsetPx absorbs the ink-estimate error (baseline−0.8em), textShareMin keeps deliberate top-aligned panel headers out; measured across the full svg fixture set: 62 guard-passing candidates, 1 fire (see FP suite in test/vuln-label-baseline-off.test.mjs)
  'anchor-buried': { minDepthPx: 8, maxNodes: 400 }, // a connector endpoint ≥8px inside a node shape (fiv boundary-clip rule) — measured separation gap [5px legit joint, 60px centre-connect target]; needs the svg adapter's `endpoints` capture, bails gracefully without it (see FP suite in test/vuln-anchor.test.mjs)
};
...
export const TYPE_SUPPRESSIONS = {
  ...
  diagram: ['no-focal-point', 'even-split', 'hanging-header', 'icon-fill-mix', 'low-contrast-ui', 'sibling-misalign', 'grid-drift'], // multi-node graph, no single focal; left-margin layer labels are legitimate; mixed node glyphs and low-contrast grid/panel chrome are deliberate; staggered node layout is intended
  poster: ['all-caps-text', 'grid-drift'],  // display typography set in caps is the medium's language, not a readability defect; asymmetric collage banding is intended
};
...
function scanContext(alt) {
  ...
  // Text carrying its RENDERED string. `label` is glyph content for a text node, but adapters
  // reuse it as an identifier otherwise: pptx.mjs sets `label = text || shape-id || 'shape'` and
  // hands EVERY shape a heading/body role, so a role alone would promote an untexted <p:sp> named
  // CONTENT_PANEL into "copy" and invite typography findings about a shape id. A node explicitly
  // kinded non-text is therefore never copy; the role branch is kept only for hand-authored ALTs
  // that omit `kind`.
  const texts = nodes.filter((n) => typeof n.label === 'string'
    && (n.kind === 'text' || (n.kind == null && TEXT_ROLES.has(n.style?.role))));
  return { nodes, canvas, weights, fontSizes, colored, gaps, icons, texts };
}
...
export const SIGNATURES = [
  ...
  },
  {
    ...
    detect(ctx, t) {
      ...
      for (let i = 0; i < ctx.nodes.length; i++) {
        ...
        // Backdrop lookup is O(n) per node, so resolve it LAST — only for the few candidates that
        // survive the shape and repetition guards. Precomputing it for every node made the whole
        // scan ~30% slower on a 6k-node import while one signature reads it.
        const j = backdropIndexFor(ctx.nodes, i);
  ...
  {
    id: 'sibling-misalign',
    title: 'same-row text siblings start at different first baselines',
    severity: 'medium',
    needs: ['geometry'],
    detect(ctx, t) {
      // GUARDS (same shape as hanging-header's — see its FP suite):
      //   maxNodes   — dense imports bail before the O(n²) row scan (no finding, no explosion).
      //   fontSize on BOTH — a row sibling without a fontSize is an icon/box, not a text sibling;
      //                restricting to genuine text↔text pairs.
      //   fontClassRatio — a 12px label beside its 24px value is a key/value row, not two
      //                same-size siblings drifting apart; only same-font-class pairs compare.
      //   tolFont    — the defect is the FIRST baseline (a ragged BOTTOM is normal; top-aligned
      //                rows agree), so a pair passes while |Δbaseline| stays within tolerance.
      if (ctx.nodes.length > t.maxNodes) return null;
      const ns = ctx.nodes;
      const pairs = [];
      for (let i = 0; i < ns.length; i++) {
        const a = ns[i];
        const aFs = Number(a.style?.fontSize);
        if (!a.bbox || !Number.isFinite(aFs)) continue;          // text siblings only
        for (let j = i + 1; j < ns.length; j++) {
          const b = ns[j];
          const bFs = Number(b.style?.fontSize);
          if (!b.bbox || !Number.isFinite(bFs)) continue;
          if (Math.max(aFs, bFs) / Math.min(aFs, bFs) > t.fontClassRatio) continue; // label/value mix
          const yO = Math.min(a.bbox.y + a.bbox.h, b.bbox.y + b.bbox.h) - Math.max(a.bbox.y, b.bbox.y);
          const minH = Math.min(a.bbox.h, b.bbox.h);
          if (minH <= 0 || yO / minH < t.yOverlap) continue;      // same row band
          const l = a.bbox.x <= b.bbox.x ? a : b;
          if (l.bbox.x + l.bbox.w > (l === a ? b : a).bbox.x) continue; // side-by-side siblings
          const dBase = Math.abs((a.bbox.y + aFs * 0.8) - (b.bbox.y + bFs * 0.8));
          if (dBase <= Math.max(aFs, bFs) * t.tolFont) continue;  // tops agree → pass
          pairs.push([a.id, b.id]);
        }
      }
      if (pairs.length < t.minPairs) return null;
      return { signal: pairs.length, threshold: t.minPairs, nodes: pairs.flat().slice(0, 4),
        remediation: 'align the FIRST baselines of text siblings in the same row (top-align, or center each column on a shared line box) — a ragged bottom edge is fine, a ragged top reads as sloppiness' };
    },
  },
  {
    id: 'grid-drift',
    title: 'section left edges drift across bands (no shared grid line)',
    severity: 'low',
    needs: ['geometry'],
    detect(ctx, t) {
      const { w: cw, h: ch } = ctx.canvas;
      if (!(cw > 0) || !(ch > 0)) return null;
      const leftTol = t.leftTolRatio * cw;
      // cut the canvas into vertical bands wherever the y-gap between consecutive leaves
      // (y-sorted) exceeds the band threshold
      const nodes = [...ctx.nodes].sort((a, b) => (a.bbox.y - b.bbox.y) || (a.bbox.x - b.bbox.x));
      const gapCut = t.bandGapRatio * ch;
      const bands = [[]];
      for (let i = 0; i < nodes.length; i++) {
        if (i > 0 && nodes[i].bbox.y - (nodes[i - 1].bbox.y + nodes[i - 1].bbox.h) > gapCut) bands.push([]);
        bands[bands.length - 1].push(nodes[i]);
      }
      // a CENTERED band (hero) and a FULL-BLEED band are display structure, not grid sections —
      // skip them; only content bands vote on the shared left edge
      const lefts = [];
      const evidence = [];
      for (const band of bands) {
        if (!band.length) continue;                                 // empty band guard: no nodes → no left, no NaN centroid
        const area = (n) => fin(n.bbox.w) * fin(n.bbox.h);
        let wsum = 0; let cxw = 0; let left = Infinity; let right = -Infinity;
        for (const n of band) {
          const a = area(n);
          wsum += a; cxw += a * (n.bbox.x + n.bbox.w / 2);
          left = Math.min(left, n.bbox.x); right = Math.max(right, n.bbox.x + n.bbox.w);
        }
        const mid = wsum > 0 ? cxw / wsum : (left + right) / 2;
        if (Math.abs(mid - cw / 2) <= leftTol) continue;            // centered band (hero)
        if (left <= leftTol && right >= cw - leftTol) continue;     // full-bleed band
        // dominant content-left: area-weighted mode of the band's left edges (±leftTol cluster);
        // strict > keeps the first anchor on a tie → deterministic under the y/x sort above.
        // In-band this answers "which of many leaves is the band's dominant line?"
        let dom = null;
        for (const n of band) {
          let vote = 0;
          for (const m of band) if (Math.abs(m.bbox.x - n.bbox.x) <= leftTol) vote += area(m);
          if (dom === null || vote > dom.vote) dom = { left: n.bbox.x, vote };
        }
        lefts.push(dom.left);
        evidence.push(...band.map((n) => n.id));
      }
      if (lefts.length < t.minBands) return null;
      // CROSS-band, a different question: "do the dominant lines share one grid?" — sub-pixel
      // exact. A transformed import yields 40.0000001 where the source said 40, so distinct is
      // keyed on 0.1px (the same rounding low-contrast-ui's tileKey applies to float noise).
      const distinct = new Set(lefts.map((v) => v.toFixed(1))).size;
      if (distinct <= t.maxLefts) return null; // two alternating lefts are a deliberate rhythm
      return { signal: distinct, threshold: t.maxLefts, nodes: evidence.slice(0, 4),
        remediation: 'snap each section to a shared left grid line (or a deliberate two-column rhythm) — every band starting at its own x reads as drift, not layout' };
    },
  },
  {
    id: 'label-baseline-off',
    title: 'sole label rides high/low in its box (absolute baseline misalignment)',
    severity: 'medium',
    needs: ['geometry'],
    detect(ctx, t) {
      // GUARDS (sibling-misalign family — see its FP suite). Sibling comparison needs TWO texts
      // in a row; a box with a single label has no row sibling, so the ABSOLUTE box centre is the
      // only reference — the structural blind spot this signature covers (baseline-off.svg: rect
      // centre 80, ink centre 76.4, Δ=−3.6 = 10% of the box height; the correct baseline 84.2
      // sits 0.6px off).
      //   maxNodes   — dense imports bail before the O(T×N) containment scan (no finding, no
      //                explosion).
      //   single-text — multi-text boxes are key/value rows and multi-line labels (each text is
      //                placed deliberately); only a SOLE label is judged against the box centre.
      //   textShareMin — a small label high in a tall panel is deliberate top-alignment, not
      //                drift; the label must fill a real share of the box height to compare.
      //   NO TYPE_SUPPRESSIONS entry: diagrams are this defect's home domain (the target fixture
      //                IS a diagram) and posters rarely carry single-text boxes — register one
      //                only on FP evidence (engine principle: FP rate is failure mode #1).
      //   INK ESTIMATE — a text bbox approximates rendered ink (top = baseline − 0.8em, height =
      //                em; lib/adapters/svg.mjs), ~half an em of metric error at the extremes,
      //                absorbed by minOffsetPx before the proportional threshold applies.
      if (ctx.nodes.length > t.maxNodes) return null;
      const isText = (n) => n.kind === 'text' || (n.kind == null && TEXT_ROLES.has(n.style?.role));
      const boxes = ctx.nodes.filter((n) => !isText(n));
      let worst = null;
      for (const text of ctx.texts) {
        // smallest containing non-text node (strict < keeps the first on a tie → deterministic
        // under the stable nodes order)
        let box = null;
        for (const b of boxes) {
          if (!containsBox(b.bbox, text.bbox)) continue;
          if (!box || b.bbox.w * b.bbox.h < box.bbox.w * box.bbox.h) box = b;
        }
        if (!box) continue;
        if (ctx.texts.filter((x) => containsBox(box.bbox, x.bbox)).length !== 1) continue; // key/value·multi-label = intent
        if (!(box.bbox.h > 0) || text.bbox.h < t.textShareMin * box.bbox.h) continue; // top-aligned header = intent
        const delta = center(text.bbox)[1] - center(box.bbox)[1];
        if (Math.abs(delta) < Math.max(t.minOffsetPx, t.offsetRatio * box.bbox.h)) continue;
        if (!worst || Math.abs(delta) > Math.abs(worst.delta)) worst = { delta, text, box };
      }
      if (!worst) return null;
      return { signal: Number(Math.abs(worst.delta).toFixed(2)),
        threshold: Number(Math.max(t.minOffsetPx, t.offsetRatio * worst.box.bbox.h).toFixed(2)),
        nodes: [worst.text.id, worst.box.id],
        remediation: `centre the sole label on its box — it rides ${worst.delta < 0 ? 'high' : 'low'} by ${Math.abs(worst.delta).toFixed(1)}px. Text bboxes are ink estimates (baseline − 0.8em), so offsets within the 2px measurement floor are estimation noise, not drift` };
    },
  },

  // ---- diagram connector-anchor signature (externalized: lib/vuln-sigs/anchor.mjs) ----
  ANCHOR_BURIED,
];
