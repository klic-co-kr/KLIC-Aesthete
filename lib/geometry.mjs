...
// PRECEDING (lower in the paint stack) filled, non-text node that fully contains it.
// Deterministic, geometry-only — no rendering. Returns the index, or -1 when nothing encloses it
...
export function backdropIndexFor(nodes, i) {
  ...
  for (let j = i - 1; j >= 0; j--) {
    ...
    if (containsBox(cand.bbox, subject.bbox)) return j;
  }
  return -1;
}
