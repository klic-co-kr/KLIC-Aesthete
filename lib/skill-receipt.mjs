...
function normalizeCurrentFlags(flags = {}) {
  ...
  const optionalString = (key, { rejectFlag = false } = {}) => {
  ...
  return {
    ...
    domain,
    slide,
    profile: optionalString('profile', { rejectFlag: true }),
...
export async function verifyReceiptFiles(input, deps = {}) {
  if (!isRecord(input)) currentInputError('verification input must be an object');
  const flags = normalizeCurrentFlags(input.flags || {});
