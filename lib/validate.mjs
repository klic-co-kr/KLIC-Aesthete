...
export function validateCorpus(corpus) {
  ...
  // Spearman on a small n is unstable (a perfect vote-order on 4 items gives rho=1.0 that means
  // nothing). Flag small samples so a high rho can't be misread as validation.
  const SMALL_N = 30;
  const smallSample = pairs.length > 0 && pairs.length < SMALL_N;
  return {
    schema_version: 1,
    n: pairs.length,
    baseline: baseline(),
    ...
    smallSample, // true when n < 30 — correlation is exploratory, NOT validation
...
async function main() {
  ...
  if (result.smallSample) console.log(`WARNING: n=${result.n} < 30 — correlation is UNSTABLE and exploratory only; a high rho here is NOT validation (a perfect synthetic vote-order on a few items gives rho=1.0 that means nothing).`);
