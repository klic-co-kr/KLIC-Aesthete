...
| `svg` | ✅ | ⚠ | import: `<path>` 포함 도형 bbox 추출, translate 누적. export는 **ALT 재출력**(원본 패치 아님) — `circle`/`ellipse`/`rect`는 보존, `<path>` Bézier·gradient·transform·stroke는 bbox-rect로 평탄화 |
