...
- **SVG / PPTX**: **import(파싱)**는 도형 좌표가 명시 → 완전 결정론적. 단 **export는 원본 패치가 아니라 ALT 재출력**이다 — `circle`/`ellipse`/`rect`는 형상을 보존하나, `<path>`의 Bézier 곡선·`<line>`·gradient·transform·stroke-width/대시 등 세부 스타일은 bbox-rect로 **평탄화**된다(원본 형상 손실). 즉 "SVG 미학 보정"은 "ALT 기반 최소 SVG 재구성"이지 원본 SVG를 그대로 되돌려주는 것이 아니다. PPTX export는 단일 슬라이드 최소 패키지(마스터·테마·미디어·차트 미포함, §8).
- **아이콘/선-아트의 collision 위양성 방지**: 루시드(lucide) 같은 stroke-only 아이콘은 `fill="none"`이라 어댑터가 `style.filled=false`로 기록. `collision`(과 fixer의 `resolveP0`)은 **둘 다 unfilled(stroke)인 쌍의 교차는 collision에서 제외** — 선-아이콘에서 교차가 의도(asterisk·snowflake·anchor)이기 때문. 채워진 영역 겹침(레이아웃 본령)은 그대로 잡는다(`filled` 기본값 true → 기존 ALT 영향 없음). 단일-path 아이콘(루시드 대다수)은 노드 1개라 애초에 측정 못 함 — 아이콘은 기본 도메인 밖(§0).
