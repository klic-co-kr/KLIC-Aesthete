...
| **2 · AI-slop signatures** | generated-UI tells — cliché gradient, glassmorphism, side-tab border, candy-hue Tailwind tinting, body emoji, bounce easing, hover transform, pulse, marquee, blink cursor, marketing lexicon, trusted-by, hero trio, lorem-ipsum, em-dash copy, oversized figures, apologetic errors, "LIVE" clock eyebrows, default-reach fonts, hollow type, dark-SaaS glow, blob radii, repeating stripes, transition:all / layout-property motion, box-drawing dividers, placeholder/broken image src, … | raw HTML source (literal-presence) | `lib/slop.mjs` (+ `lib/slop/signatures/`) | 39 signatures across 5 sub-axes (palette · decoration · copy · template · imagery); 18 of them ported from [@gessobuild/anti-slop](https://github.com/Gesso-Build/skills) v0.4.2 (MIT) |
...
The second axis is a literal-presence HTML scanner — it reads raw source (CSS, copy, structure), not the ALT geometry tree, because most AI tells (cliché gradients, motion easing, marketing lexicon) live in the source grammar that geometry cannot see. **v1 is HTML-only**; SVG `<animate>` / PPTX `<p:timing>` / React TSX-JSX source / raster-image tells need their own calibrated scanners (v2 — see [`docs/superpowers/specs/2026-07-23-slop-v2-medium-expansion.md`](./docs/superpowers/specs/2026-07-23-slop-v2-medium-expansion.md)). Signatures are organized by four sub-axes; thresholds are conservative presence floors (uncalibrated until the v2 human-corpus lands).

| Sub-axis | Signatures | Top tells |
|---|---|---|
| `palette` | 4 | cliché indigo→violet→pink gradient · glassmorphism (`backdrop-filter`) · gradient border (card top bar / callout left rail) · **candy-hue Tailwind tinting** (emerald/amber pill badges + inline color spans — one legit status tint stays silent) |
| `decoration` | 11 | emoji-in-heading · **emoji-in-body** (subdivision flags excluded) · italic heading (Hallmark gate 38a) · icon saturation · decorative scale/rotate animation · **side-tab border** (Impeccable's #1 AI-UI tell — P0, rule-block asymmetry: ≥2px on one side while the others stay hairline; replace with a hover lift or internal tag badge) · **bounce easing** · **hover transform** · **pulse** · **marquee** · **blink cursor** |
...
lib/
  ...
  slop.mjs       AI-slop signature engine — mirrors vuln (signature fold + overridable thresholds + advisory). v1 = HTML literal-presence scan only (SVG/PPTX/TSX-source/LLM-judge = v2). Four axes: palette (cliché indigo→violet→pink gradient · glassmorphism · gradient border [card top bar / callout left rail] · candy-hue Tailwind tinting [emerald/amber pill badges + inline color spans, issue #1 — neutral + legacy hues exempt, evidence must span ≥2 elements]), decoration (emoji-in-heading · emoji-in-body [⚠/✅/🔥 in prose — subdivision flags excluded] · italic heading [Hallmark gate 38a — top AI tell] · icon saturation · decorative animation · side-tab border [Impeccable's #1 AI-UI tell: ≥2px on one side of a card while the others stay hairline — P0 since issue #2; rule-block scoped with sequential shorthand application, so full frames / double rails / 1px dividers stay silent; remediation steers to a translateY hover lift or internal tag badge] · bounce easing · hover transform · pulse · marquee · blink cursor), copy (LLM marketing lexicon + fake-precision metrics [many-9 % / round Nx — research-attested tells] over body+headings, separator-normalized; generic LLM-judge = v2 stub → always unmeasured), template (trusted-by logo strip · hero trio). Uncalibrated — conservative presence floors, corpus tuning is v2
